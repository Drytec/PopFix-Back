...
export async function addUserMovieComment(req: Request, res: Response) {
  try {
    ...
    const user_surname: string =
      user_data.surname && user_data.surname !== "N/A" ? user_data.surname : "";
...
export async function deleteFavorite(req: Request, res: Response) {
  try {
    ...
    const updated = await updateUserMovie(userId, movieId, {
      is_favorite: false,
    });
    ...
    for (const c of commentsRaw) {
      try {
        ...
        const author_surname =
          author?.surname && author.surname !== "N/A" ? author.surname : "";
        const avatar =
          c.avatar ||
          (author_name && author_surname
            ? (author_name[0] + author_surname[0]).toUpperCase()
            : author_name
...
export async function setRating(req: Request, res: Response) {
  try {
    ...
    const sugestedRatings = await getRatingMovies(movieId);
    console.debug("[DEBUG] setRating computed suggestedRatings:", {
      movieId,
      sugestedRatings,
      userId,
      incomingRating: rnum,
    ...
    if (typeof sugestedRatings === "number" && sugestedRatings >= 1) {
      ...
      if (!existing) {
        try {
          ...
          try {
            newMovie = await updateMovieById(movieId, {
    ...
    return res
      .status(200)
      .json({
        message: "Listo, tu valoración ha sido guardada",
        data: result,
        userRating: rnum,
        suggestedRating: sugestedRatings,
        updatedMovie: newMovie,
